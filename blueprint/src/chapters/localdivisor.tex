...
\begin{definition}[Local divisor]\label{def:localdiv}
  \lean{KRTheory.TransMon.LocalDivisor}\leanok
...
\begin{lemma}[{[DKS] 2.13}: faithfulness]\label{lem:localdiv-faithful}
  \lean{KRTheory.TransMon.localDivisor_faithful}\leanok
...
\begin{lemma}[Cardinality drop]\label{lem:localdiv-card}
  \lean{KRTheory.TransMon.localDivisor_card_lt}\leanok
...
\begin{lemma}[Division]\label{lem:localdiv-divides}
  \lean{KRTheory.TransMon.localDivisor_divides}\leanok
  \uses{def:localdiv,def:mdiv}
  $M_c \prec_{\mathrm m} M$. This is what makes the strong form of Krohn--Rhodes
...
\begin{lemma}[Nonempty local states]\label{lem:localdiv-nonempty}
  \lean{KRTheory.TransMon.localDivisor_X_nonempty}\leanok
  \uses{def:localdiv-tm}
  If $X \neq \emptyset$ then $X_c \neq \emptyset$.
\end{lemma}
\begin{proof}
  \leanok
