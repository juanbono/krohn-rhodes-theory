...
\begin{definition}[Semigroup division]\label{def:semdiv}
  \lean{KRTheory.SemigroupDivides}\leanok
  \uses{def:mdiv}
...
\begin{lemma}[Reflexivity and transitivity]\label{lem:semdiv-preorder}
  \lean{KRTheory.SemigroupDivides.refl, KRTheory.SemigroupDivides.of_subsemigroup, KRTheory.SemigroupDivides.trans}\leanok
  \uses{def:semdiv}
...
\begin{lemma}[Monoid division is semigroup division]\label{lem:semdiv-of-mdiv}
  \lean{KRTheory.monoidDivides_semigroupDivides}\leanok
  \uses{def:semdiv,def:mdiv}
  $M \prec_{\mathrm m} N$ implies $M \prec_s N$.
\end{lemma}
\begin{proof}
  \leanok
...
\begin{lemma}[Adjoining an identity]\label{lem:withone-transfer}
  \lean{KRTheory.withOne_transfer}\leanok
  \uses{def:semdiv}
...
\begin{proof}
  ...
  elements of the copy of $S$ inside $S^1$ is again one), and the
  inverse of the injection $S \hookrightarrow S^1$ is a surjective
  homomorphism onto $S$.
\end{proof}

\begin{lemma}[Factor transport]\label{lem:semdiv-group-withone}
  \lean{KRTheory.semigroupDivides_of_monoidDivides_withOne}\leanok
  \uses{def:semdiv,def:mdiv}
  Let $G$ be a \emph{nontrivial} finite group and $S$ a semigroup. If
  $G \prec_{\mathrm m} S^1$ then $G \prec_s S$.
\end{lemma}
\begin{proof}
  \leanok
  Let $N \le S^1$ be a submonoid with a surjection $\psi : N \to G$,
  and let $T := \{\, x \in S \mid x \in N \,\}$, a subsemigroup of $S$
  (a product of two elements of the copy of $S$ inside $S^1$ is again
  one).
...
\begin{theorem}[Krohn--Rhodes, semigroup form]\label{thm:krohnrhodes-semigroup}
  \lean{KRTheory.krohnRhodes_semigroup}\leanok
...
\begin{proof}
  ...
  $S^1$, giving $S^1 \prec_{\mathrm m} W$ for the wreath monoid $W$ of some
  ...
  Lemma~\ref{lem:semdiv-group-withone} converts $G \prec_{\mathrm m} S^1$ into
  $G \prec_s S$.
\end{proof}
