...
\begin{definition}[Regular representation]\label{def:regular}
  \lean{KRTheory.TransMon.regular}\leanok\uses{def:transmon}
...
\begin{lemma}\label{lem:regular-faithful}
  \lean{KRTheory.TransMon.regular_faithful}\leanok\uses{def:regular,def:faithful}
...
\begin{lemma}\label{lem:trivialTM-faithful}
  \lean{KRTheory.TransMon.trivialTM_faithful}\leanok\uses{def:trivialTM,def:faithful}
  The trivial transformation monoid is faithful.
\end{lemma}
\begin{proof}
  \leanok
  $M$ has only one element, so there is nothing to distinguish.
\end{proof}
